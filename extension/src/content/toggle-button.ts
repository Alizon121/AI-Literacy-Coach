...
export interface ToggleButtonCallbacks {
  onGetFeedback: () => void;
  onTogglePause: () => void;
}
...
export function createToggleButton(
  inputEl: HTMLElement,
  callbacks: ToggleButtonCallbacks
): ToggleButton {
  ...
  const handleOutsideClick = () => { if (menuOpen) closeMenu(); };
  const handleEscape = (e: KeyboardEvent) => { if (e.key === "Escape" && menuOpen) closeMenu(); };
...
function makeMenuItem(label: string, isDanger = false): HTMLButtonElement {
